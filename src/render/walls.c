...
/*   Updated: 2026/01/23 03:01:26 by ivmirand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "render.h"

static void	build_wall_bounds(t_rayhit *rayhit, mlx_image_t *img, t_player * player)
{
	int			slice_height;
	...
	center = (int)(img->height * 0.5f + player->pitch);
...
static void	render_wall_fill(t_rayhit *rayhit, unsigned int x, mlx_image_t *img,
		t_textures *textures, t_player *player)
{
	int			screen_bounds[2];

	if (!rayhit->hit)
		return ;
	build_wall_bounds(rayhit, img, player);
