...
/*   Updated: 2026/02/08 13:12:42 by marcnava         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Collision bonus
bool				is_cell_blocking_bonus(t_game *game, int cell_x,
						int cell_y);
bool				collides_with_wall_bonus(t_game *game, float x, float y);
void				attempt_move_bonus(t_game *game, float step_x,
						float step_y);
...
bool				apply_headbob_pitch(t_game *game, bool is_moving,
						bool step_started, float prev_headbob);
void				read_move_axes(t_game *game, float *forward, float *strafe);
bool				apply_translation_input(t_game *game, float forward,
						float strafe);
...
bool				update_step_audio_and_headbob(t_game *game,
						bool player_translated, float prev_headbob);
bool				update_jump_pitch(t_game *game, float prev_jump);
bool				handle_modal_loop(t_game *game);
bool				throttle_fps_loop(t_game *game, double *next_tick,
						int *last_limit);
...
bool				orb_projectile_start_take(t_game *game, int cell_x,
						int cell_y, char block);
bool				orb_projectile_start_place(t_game *game, int target_x,
						int target_y, char block);
bool				orb_projectile_start_elevator_place(t_game *game,
						const int target_cell[2], char block,
						int elevator_slot);
...
bool				orb_projectile_spawn_ghost(t_game *game, int target_x,
						int target_y);
...
void				config_modal_handle_key(t_game *game,
...
bool				controller_poll_state(t_game *game,
...
float				controller_action_value(t_game *game, int action,
						const GLFWgamepadstate *state, float deadzone);
bool				controller_prepare_update(t_game *game,
						GLFWgamepadstate *state,
...
bool				controller_menu_handle_left_column(t_game *game,
						const bool in[7]);
bool				controller_menu_handle_controls(t_game *game, bool up,
						bool down, bool confirm);
void				controller_menu_handle_settings(t_game *game,
						const bool in[7]);
...
bool				state_target_intersects_player_bonus(t_game *game,
						int x, int y);
bool				state_is_valid_target_cell_bonus(t_game *game,
						int x, int y);
bool				state_modify_interactive_cell(t_game *game, int x, int y);
bool				state_get_attachment_coords_bonus(t_game *game,
						t_rayhit *hit, int *x, int *y);
bool				state_handle_elevator_orb_place_bonus(t_game *game,
						t_rayhit *hit, char cell);
bool				state_get_place_hit_bonus(t_game *game, t_rayhit *hit,
						char *cell);
